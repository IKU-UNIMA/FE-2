                                ...
                                return (
                                    <>
                                        <div className="flex mr-2 mb-2">
                                            <div className="p-2 h-[10px] my-auto rounded-full mr-4" style={{ "backgroundColor": `${COLORS[index % COLORS.length]}`, }}></div>
            ...
            <div className="flex">
                    ...
                    <div className="flex justify-center">
                        ...
                        <div className="text-center mx-auto justify-center w-[50%] my-auto">
                            {persentageData.map((datas, index) => {
                                ...
                                const total = JSON.stringify(datas.total / pieData.reduce(reducer) * 100)
                                return (
                                    <>
                                        <div className="flex mr-2 mb-2">
                                            <div className="p-2 h-[10px] my-auto rounded-full mr-4" style={{ "backgroundColor": `${COLORS[index % COLORS.length]}`, }}></div>
                                            ...
                                            <div className="flex w-[90%] justify-between">
                                                ...
                                                <div className="">{datas.total} ({total.slice(0, 4)}{total.length > 2 ? total.slice(total.length - 1, total.length) : null}%)</div>
                                            </div>
                                        </div>
                                        <div className="h-[1px] w-full bg-black mb-2"></div>
                                    </>
                ...
                <div className="w-[25%] h-full pl-6">
                    {persentageData.map((data) => (
                        <>
                            <div id="persentageTotal" className="flex flex-col shadow-lg p-4 mx-auto justify-center items-center rounded-lg bg-white">
    ...
    const CustomTooltip = ({ active, payload, label }) => {
        ...
        if (active) {
            ...
            return (
                ...
                >
                    <div className="">
                        <div className="">{`${payload[0].name} : ${(payload[0].value / pieData.reduce(reducer) * 100)}%`}</div>
                        {/* <div className="">{`${payload[0].name} : ${(payload[0].value / 1000 * 100)}%`}</div> */}
                        <div>{`${payload[0].value}`} students</div>
                    </div>
                </div>
        ...
        <>
            <div className="">
                <div className="w-[100%] bg-white p-4 rounded-t-xl shadow-lg">
                    <h1 className="text-[20px] mb-2 font-medium tracking-[1px]">{identify}</h1>
                            ...
                            persentageData.length !== 0 ? (
                                <>
                                    ...
                                    <div className="text-center mx-auto justify-center w-[60%] mt-4 my-auto">
                                        {persentageData.map((datas, index) => {
                                            const total = JSON.stringify(datas.jumlah / pieData.reduce(reducer) * 100)
                                            return (
                                                <>
                                                    <div className="flex mr-2 mb-2">
                                                        <div className="p-2 h-[10px] my-auto rounded-full mr-4" style={{ "backgroundColor": `${COLORS[index % COLORS.length]}`, }}></div>
                                                        ...
                                                        <div className="flex w-[100%] justify-between">
                                                            <div className="mr-1">{datas.nama}</div>
                                                            <div className="">{datas.jumlah} ({total.slice(0, 4)}{total.length > 2 && persentageData.length !== 1 ? total.slice(total.length - 1, total.length) : null}%)</div>
                                                        </div>
                                                    </div>
                                                    <div className="h-[1px] w-full bg-black mb-2"></div>
                                                </>
                ...
                <div className="p-4">
                    ...
                    <table className="w-[100%] rounded-md mt-4">
                        ...
                        {persentageData.length === 0 ? (
                            ...
                            <>
                                <tr>
                                    <td className="border-[2px] border-black px-2">{data.prodi?.jenjang} {data.prodi?.nama}</td>
                                    <td className="border-[2px] border-black px-2">{data.fakultas?.nama}</td>
                                    <td className="text-center border-[2px] border-black px-2">{data.Semester?.nama}</td>
                                    <td className="text-center border-[2px] border-black px-2">{data.jumlah}</td>
                                </tr>
                            </>
                        ))
                        }
        ...
        <>
            <div className="">
                {loading ? (
                    ...
                    <>
                        <div className="w-[100%] grid grid-cols-2 gap-6 h-full mb-6">
                            <div id="persentageTotal" className="w-full relative flex flex-col hover: shadow-lg px-4 py-8  mx-auto justify-center items-center rounded-lg bg-white">
                                <div onClick={() => setEditTarget(!editTarget)} className="absolute top-0 right-0 mr-2 mt-2">{editTarget === false ? <BsPencilSquare /> : "X"}</div>
                                <h1 className="text-[20px] font-medium">Target</h1>
                                {editTarget === false ? (
                                    <>
                                        <h1 className="font-medium text-[24px]">{persentageData.target}%</h1>
                                    </>
                                ) : (
                                    <>
                                        <div className="w-[50%]">
                                            <input onChange={(e) => setTargetPencapaian(Number(e.target.value).toFixed(1))} className="w-full border-[1px] border-black mt-1" type="number" />
                            ...
                            <div id="persentageTotal" className="w-full flex flex-col shadow-lg px-4 py-8 mx-auto justify-center items-center rounded-lg bg-white">
                                <h1 className="text-[20px] font-medium">Pencapaian</h1>
                                <h1 className="font-medium text-[24px]">{persentageData.pencapaian}</h1>
                                <h1 className="text-[14px] text-center">Dengan total {persentageData.total} dari {persentageData.total_mahasiswa} mahasiswa</h1>
                            </div>
                        </div>
                        <div className="w-[100%] bg-white p-4 rounded-xl shadow-lg  mr-6">
                            <div className="flex justify-between">
                                {
                                    tahun == 0 ? (
                                        <h1 className="text-[20px] mb-2 font-medium tracking-[1px]">Data Fakultas (Semua)</h1>
                                    ) : (
                                        <h1 className="text-[20px] mb-2 font-medium tracking-[1px]">Data Fakultas ({tahun})</h1>
                                    )
                                }
                            </div>
                            <div className="flex flex-col justify-center">
                            ...
                            <>
                                ...
                                <div className="flex pb-1 pt-1">
                                    ...
                                    <div className="w-[10%] mx-2">
                                        {identify === "admin" && (
                                            <button onClick={() => navigate(`/dashboard/admin/statistik/${data.id}`)} className="border-[1px] border-black text-black px-2 my-auto text-[14px]">Lihat</button>
                                        )}
                                        {identify === "rector" && (
                                            <button onClick={() => navigate(`/iku/${data.id}`)} className="border-[1px] border-black text-black px-2 my-auto text-[14px]">Lihat</button>
                                        )}
                                    </div>
                                </div>
                            </>
            ...
            <div className="">
                <div className="w-[100%] bg-white p-4 rounded-xl shadow-lg">
                    <div className="flex justify-between">
                        {
                            tahun == 0 ? (
                                <h1 className="text-[20px] mb-2 font-medium tracking-[1px]">Data {persentageData.fakultas} (Semua)</h1>
                            ) : (
                                <h1 className="text-[20px] mb-2 font-medium tracking-[1px]">Data {persentageData.fakultas} ({tahun})</h1>
                            )
                        }
                    </div>
                    <div className="flex flex-col justify-center">
